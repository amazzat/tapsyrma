...
export function CreateProjectModal({ show, onHide }) {
  ...
  return (
    show && (
      <div
        className="fixed inset-0 flex h-screen w-full items-center justify-center bg-black bg-opacity-50 p-5 backdrop-blur-sm"
        onClick={onHide}
      >
        <form
          className="flex flex-col space-y-5 rounded border-neutral-700 bg-neutral-800 px-12 py-10 shadow-inner"
          onClick={(e) => e.stopPropagation()}
          onSubmit={handleSubmitCreateProject}
        >
          <label className="w-96" htmlFor="text">
            <div className="input-label mb-6 text-3xl font-bold ">
              Enter project name
            </div>
            <input
              type="text"
